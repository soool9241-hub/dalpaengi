...
const CATS=[...new Set(T.map(t=>t.cat))];
const PHASES=[...new Set(T.map(t=>t.phase))];
...
const PC={"모객":"#E8593C","응답":"#3DB88C","견적":"#5B9FE4","클로징":"#E4A84B","팔로업":"#9B8FE8","입금":"#E4A84B","입실":"#5B9FE4","퇴실":"#9B8FE8","이슈":"#E85D5D"};
const PFS=["전체","MT","가족모임","종교모임","워크샵","지인모임","동호회","숙박"];
const SFLS=["전체","upcoming","visited","cancelled"];
const SL={"upcoming":"예정","visited":"방문완료","cancelled":"취소"};
const SCO={"upcoming":"#5B9FE4","visited":"#3DB88C","cancelled":"#E85D5D"};
const bg0="#080808",bg1="#0f0f0e",bg2="#171716",bg3="#1e1e1c",bdr="#2d2d2a",tx1="#e8e6df",tx2="#9c9a92",tx3="#5e5d58",grn="#40916C",grnL="#5aad86";

function Pill({on,color,children,style:s,...p}){
  return <button{...p}style={{padding:"8px 16px",fontSize:13,fontWeight:600,borderRadius:10,border:"none",cursor:"pointer",transition:"all .15s",background:on?(color||tx1):bg3,color:on?(color?"#fff":bg0):tx2,...(s||{})}}>{children}</button>;
}
...
export default function SmsPage(){
  ...
  useEffect(()=>{if(step===1)loadC();},[step,pF,sF]);

  const filtered=useMemo(()=>{
    let r=T;
    if(phaseF)r=r.filter(t=>t.phase===phaseF);
    if(catF)r=r.filter(t=>t.cat===catF);
    if(tplQ.trim()){const q=tplQ.toLowerCase();r=r.filter(t=>(t.title+t.msg+t.cat+t.phase).toLowerCase().includes(q));}
    return r;
  },[tplQ,catF,phaseF]);
  ...
  return(
    <div style={{fontFamily:"'Pretendard',system-ui",maxWidth:720,margin:"0 auto",background:bg0,minHeight:"100vh",color:tx1}}>
      <div style={{padding:"28px 24px 22px",borderBottom:`1px solid ${bdr}`}}>
        ...
        <div style={{fontSize:14,color:tx2,marginTop:6}}>전환율 극대화 메시지 · {T.length}개 템플릿</div>
      </div>

      <div style={{display:"flex",gap:4,padding:"16px 24px 20px"}}>
        {["템플릿","고객선택","발송"].map((l,i)=>{const done=i<step,on=i===step;return(
          <button key={i} onClick={()=>{if(i===0)setStep(0);if(i===1&&sel)setStep(1);if(i===2&&sel&&picks.length)setStep(2);}}
            style={{flex:1,padding:"14px 0",fontSize:15,fontWeight:700,borderRadius:12,border:"none",cursor:"pointer",
              background:on?grn:done?grn+"20":bg2,color:on?"#fff":done?grnL:tx3}}>{done?"✓ ":""}{l}</button>);})}
      </div>

      <div style={{padding:"0 24px 60px"}}>
        {/* ═══ STEP 0 ═══ */}
        {step===0&&(<>
          <input placeholder="🔍 상황 검색 (빈방, MT, 클로징, 비싼데요, 리뷰...)" value={tplQ} onChange={e=>setTplQ(e.target.value)} style={{...inp,marginBottom:12}}/>

          {/* 퍼널 단계 필터 */}
          <div style={{marginBottom:8}}>
            <div style={{fontSize:12,fontWeight:600,color:tx3,marginBottom:6}}>퍼널 단계</div>
            <div style={{display:"flex",gap:5,flexWrap:"wrap"}}>
              <Pill on={!phaseF} onClick={()=>{setPhaseF(null);setCatF(null);}}>전체</Pill>
              {PHASES.map(p=><Pill key={p} on={phaseF===p} color={PC[p]} onClick={()=>{setPhaseF(phaseF===p?null:p);setCatF(null);}}>{p} {T.filter(t=>t.phase===p).length}</Pill>)}
            </div>
          </div>

          {/* 카테고리 필터 */}
          <div style={{marginBottom:16}}>
            <div style={{fontSize:12,fontWeight:600,color:tx3,marginBottom:6}}>카테고리</div>
            <div style={{display:"flex",gap:5,flexWrap:"wrap"}}>
              {(phaseF?[...new Set(T.filter(t=>t.phase===phaseF).map(t=>t.cat))]:CATS).map(c=>
                <Pill key={c} on={catF===c} color={CC[c]} onClick={()=>setCatF(catF===c?null:c)} style={{padding:"6px 12px"}}>{c}</Pill>
              )}
            </div>
          </div>

          <div style={{fontSize:13,color:tx3,marginBottom:10}}>{filtered.length}개 · 클릭→전문 보기</div>

          {filtered.map(t=>{
            const c=CC[t.cat]||"#888",isOpen=expand===t.id,isSel=sel?.id===t.id;
            return(
              <div key={t.id} style={{borderRadius:14,marginBottom:8,border:`1px solid ${isSel?c:bdr}`,background:bg1,overflow:"hidden"}}>
                <div onClick={()=>setExpand(isOpen?null:t.id)} style={{padding:"16px 18px",cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  <div style={{display:"flex",alignItems:"center",gap:8,flex:1,minWidth:0}}>
                    <span style={{fontSize:10,fontWeight:700,padding:"3px 8px",borderRadius:5,background:PC[t.phase]+"20",color:PC[t.phase],flexShrink:0}}>{t.phase}</span>
                    <span style={{fontSize:11,fontWeight:700,padding:"3px 8px",borderRadius:5,background:c+"20",color:c,flexShrink:0}}>{t.cat}</span>
                ...
                {isOpen&&(<div style={{padding:"0 18px 18px"}}>
                  ...
                  <div style={{display:"flex",gap:8,marginTop:14}}>
                    ...
                    <button onClick={e=>{e.stopPropagation();setSel(t);setMsg(t.msg);setStep(1);}} style={{flex:2,padding:12,fontSize:15,fontWeight:700,borderRadius:10,border:"none",background:grn,color:"#fff",cursor:"pointer"}}>이 템플릿으로 발송 →</button>
                  </div>
                </div>)}
              </div>);})}
        </>)}
        ...
        {step===1&&(<>
          <div style={{padding:16,borderRadius:14,background:bg1,border:`1px solid ${bdr}`,marginBottom:18,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div><div style={{fontSize:12,color:tx3}}>선택된 템플릿</div><div style={{fontSize:18,fontWeight:700,marginTop:2}}>{sel?.title}</div></div>
          ...
          <div style={{display:"flex",gap:8,marginBottom:14}}>
            <input placeholder="🔍 이름 · 전화번호" value={cQ} onChange={e=>setCQ(e.target.value)} onKeyDown={e=>e.key==="Enter"&&loadC()} style={{...inp,flex:1}}/>
            <button onClick={loadC} style={{padding:"14px 22px",fontSize:15,fontWeight:700,borderRadius:12,border:"none",background:tx1,color:bg0,cursor:"pointer"}}>{ld?"...":"검색"}</button>
          </div>
          <div style={{marginBottom:8}}><div style={{fontSize:12,fontWeight:600,color:tx3,marginBottom:6}}>예약 목적</div>
            <div style={{display:"flex",gap:5,flexWrap:"wrap"}}>{PFS.map(f=><Pill key={f} on={pF===f} onClick={()=>setPF(f)} style={{padding:"6px 12px"}}>{f}</Pill>)}</div></div>
          <div style={{marginBottom:16}}><div style={{fontSize:12,fontWeight:600,color:tx3,marginBottom:6,marginTop:8}}>상태</div>
            <div style={{display:"flex",gap:5}}>{SFLS.map(f=><Pill key={f} on={sF===f} color={SCO[f]} onClick={()=>setSF(f)} style={{padding:"6px 12px"}}>{f==="전체"?"전체":SL[f]}</Pill>)}</div></div>
          ...
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
            <div style={{fontSize:16,fontWeight:700}}>{ld?"불러오는 중...":(<>{custs.length}명{picks.length>0&&<span style={{color:grnL,marginLeft:10}}>{picks.length}명 선택</span>}</>)}</div>
            {custs.length>0&&<button onClick={pickAll} style={{padding:"8px 16px",fontSize:13,fontWeight:700,borderRadius:10,border:"none",cursor:"pointer",background:picks.length===custs.length?grn:bg3,color:picks.length===custs.length?"#fff":tx2}}>{picks.length===custs.length?"전체해제":"전체선택"}</button>}
          </div>
          <div style={{borderRadius:14,border:`1px solid ${bdr}`,overflow:"hidden",maxHeight:420,overflowY:"auto"}}>
            {custs.map((c,i)=>{const on=picks.includes(c.guest_phone),sc=SCO[c.status]||"#888";return(
              <div key={c.guest_phone+i} onClick={()=>pick(c.guest_phone)} style={{padding:"16px 18px",display:"flex",justifyContent:"space-between",alignItems:"center",cursor:"pointer",borderBottom:`1px solid ${bg3}`,background:on?grn+"08":"transparent"}}>
                <div style={{flex:1,minWidth:0}}>
                  <div style={{display:"flex",alignItems:"center",gap:8}}><span style={{fontSize:16,fontWeight:700}}>{c.guest_name}</span><span style={{fontSize:11,fontWeight:600,padding:"2px 8px",borderRadius:5,background:sc+"15",color:sc}}>{SL[c.status]||c.status}</span></div>
          ...
          {custs.length>0&&<button onClick={()=>setStep(2)} disabled={!picks.length} style={{width:"100%",padding:16,fontSize:17,fontWeight:800,borderRadius:14,border:"none",marginTop:18,cursor:picks.length?"pointer":"not-allowed",background:picks.length?grn:bg3,color:picks.length?"#fff":tx3}}>{picks.length}명에게 발송 준비 →</button>}
        </>)}

        {/* ═══ STEP 2 ═══ */}
        {step===2&&(<>
          <div style={{padding:18,borderRadius:14,background:bg1,border:`1px solid ${bdr}`,marginBottom:18}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <div><span style={{fontSize:14,color:tx2}}>발송 대상 </span><span style={{fontSize:26,fontWeight:800,color:grnL}}>{picks.length}명</span></div>
              <button onClick={()=>setStep(1)} style={{fontSize:14,fontWeight:600,color:grnL,background:"none",border:"none",cursor:"pointer"}}>수정 →</button>
            </div>
            <div style={{fontSize:14,color:tx2,marginTop:8,lineHeight:1.7,maxHeight:52,overflow:"hidden"}}>{custs.filter(c=>picks.includes(c.guest_phone)).map(c=>c.guest_name).join(", ")}</div>
          </div>
          ...
          <div style={{fontSize:14,color:tx3,marginTop:8,marginBottom:20}}><span style={{color:"#E4A84B",fontWeight:700}}>{"{변수}"}</span> 부분을 실제 값으로 수정</div>
          ...
          <button onClick={send} disabled={sending} style={{width:"100%",padding:18,fontSize:18,fontWeight:800,borderRadius:14,border:"none",cursor:sending?"not-allowed":"pointer",background:sending?bg3:grn,color:sending?tx3:"#fff"}}>
            {sending?"발송 중...":`📤 ${picks.length}명에게 Solapi 발송`}</button>
          {sendRes&&<div style={{marginTop:14,padding:18,borderRadius:14,fontSize:16,fontWeight:700,textAlign:"center",background:sendRes.ok?grn+"12":"#E85D5D12",color:sendRes.ok?grnL:"#E85D5D",border:`1px solid ${sendRes.ok?grn+"30":"#E85D5D30"}`}}>{sendRes.ok?`✅ ${sendRes.n}건 발송 완료!`:`❌ ${sendRes.e}`}</div>}
        </>)}
      </div>
    </div>
  );
